...
--           align_keys     = false,  -- if true, align the keys in a way that sounds like it should be nice,
--                                        but is actually ugly
...
--   Note that unlike
--      JSON.decodeNumbersAsObjects = true
--   this stringification is simple and unintelligent: the JSON number simply becomes a Lua string,
...
function OBJDEF:newArray(tbl)
   assert(self ~=nil, "dummy luacheck related statement")
...
-- _ parsed_value
function OBJDEF:onTrailingGarbage(json_text, location, _, etc)
...
local function json_string_literal(value, options)
   ...
   if options.stringsAreUtf8 then
      --
      -- This feels really ugly to just look into a string for the sequence of bytes
      --    that we know to be a particular utf8 character,
...
--                      The result is uglier than one might at first imagine.
...
--
--   20100810.2    added some checking to ensure that an invalid Unicode character couldn't
--                   leak in to the UTF-8 encoding
--
